                ...
                <div class="uk-flex uk-flex-middle">
                    ...
                    <select name="{module}_catalogue_id" class="form-control mr10 setupSelect2">
                        @foreach ($dropdown as $key => $value)
                            <option {{ (${module}Catalogueid == $key) ? 'selected' : '' }} value="{{ $key }}">{{ $value }}</option>
                        @endforeach
                    </select>
                    ...
                    <a href="{{ route('{module}.create') }}" class="btn btn-danger"><i class="fa fa-plus mr5"></i>Thêm mới bài viết</a>
                </div>
            </div>
        </div>
    </div>
