    ...
    <tbody>
        @if(!empty($products) && is_object($products))
            @foreach ($products as $product)
                <tr id="{{ $product->id }}">
                    <td>
                        <input type="checkbox" name="check" id="" class="input-checkbox checkBoxItem" value="{{ $product->id }}">
                    </td>
                    <td>
                    ...
                    <td class="text-center js-switch-{{ $product->id }}">
                        <input value="{{ $product->publish }}" type="checkbox" class="js-switch status" data-field="publish" data-model="{{ $config['model'] }}" {{ ($product->publish == 2) ? 'checked' : '' }} data-modelId="{{ $product->id }}" />
                    </td>
