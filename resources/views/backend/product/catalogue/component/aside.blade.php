            ...
            <div class="col-lg-12">
                <div class="form-row">
                    <span class="image img-cover image-target">
                        <img src="{{ (old('image', ($productCatalogue->image) ?? '')) ? old('image', ($productCatalogue->image) ?? '') : 'backend/img/imgnf.jpeg' }}" alt="">
                    </span>
                    ...
                    <div class="mb15">
                        <select name="publish" class="form-control setupSelect2">
                            @foreach ( __('messages.publish') as $key => $value)
                                <option {{ $key == old('publish', (isset($productCatalogue->publish)) ? $productCatalogue->publish : '') ? 'selected' : '' }} value="{{ $key }}">{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
