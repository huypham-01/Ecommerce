    ...
    <tbody>
        @if(!empty($attributes) && is_object($attributes))
            @foreach ($attributes as $attribute)
                <tr id="{{ $attribute->id }}">
                    <td>
                        <input type="checkbox" name="check" id="" class="input-checkbox checkBoxItem" value="{{ $attribute->id }}">
                    </td>
                    <td>
                    ...
                    <td class="text-center js-switch-{{ $attribute->id }}">
                        <input value="{{ $attribute->publish }}" type="checkbox" class="js-switch status" data-field="publish" data-model="{{ $config['model'] }}" {{ ($attribute->publish == 2) ? 'checked' : '' }} data-modelId="{{ $attribute->id }}" />
                    </td>
