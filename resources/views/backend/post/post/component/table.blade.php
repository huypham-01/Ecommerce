    ...
    <tbody>
        @if(!empty($posts) && is_object($posts))
            @foreach ($posts as $post)
                <tr id="{{ $post->id }}">
                    <td>
                        <input type="checkbox" name="check" id="" class="input-checkbox checkBoxItem" value="{{ $post->id }}">
                    </td>
                    <td>
                    ...
                    <td class="text-center js-switch-{{ $post->id }}">
                        <input value="{{ $post->publish }}" type="checkbox" class="js-switch status" data-field="publish" data-model="{{ $config['model'] }}" {{ ($post->publish == 2) ? 'checked' : '' }} data-modelId="{{ $post->id }}" />
                    </td>
